...
mongo = MongoClient('mongodb+srv://manasvi:man14@rc.jxjlg.mongodb.net/test', ssl_cert_reqs=ssl_support.CERT_NONE)
...
def analysis():
    ...
    if colName in mongo.test.list_collection_names():
        ...
        for s in newsub_IA:
            ...
            passing_percentageIA = '%0.2f' % ((IA_analysis/IA_appeared)*100)
        ...
        for x in newsub_P:
            ...
            pass_percp = '%0.2f' % ((p_analysis/p_appeared)*100)
        ...
        for y in newsub_SE:
            ...
            pass_percps = '%0.2f' % ((p_analysiss/p_appeareds)*100)
        ...
        passing_percentage = '%0.2f' % ((passed_students/appeared_students)*100)
        ...
        fc=mongo.test[colName].find({'$and':[{"upload_file.GPA": {"$ne": ""}},{"$expr": { "$gte": [ { "$toDouble": "$upload_file.GPA" },6.50  ]}}, {"$expr": { "$lte": [ { "$toDouble": "$upload_file.GPA" },7.75  ]}}]}).count()
        sclass=mongo.test[colName].find({'$and':[{"upload_file.GPA": {"$ne": ""}},{"$expr": { "$gte": [ { "$toDouble": "$upload_file.GPA" },5.00  ]}}, {"$expr": { "$lte": [ { "$toDouble": "$upload_file.GPA" },6.50   ]}}]}).count()
        pclass=mongo.test[colName].find({'$and':[{"upload_file.GPA": {"$ne": ""}},{"$expr": { "$gte": [ { "$toDouble": "$upload_file.GPA" },4.00 ]}}, {"$expr": { "$lte": [ { "$toDouble": "$upload_file.GPA" },5.00  ]}}]}).count()
        data = {'Particular' : 'Without KT(regular)',
             "Total no of Students with First Class with Distinction":  fc_distinction,
             "Total no of students with First Class":fc,
             "Total no of students with Second Class":sclass,
             "Total no of students with Pass Class": pclass}
        ...
        newsub_SE=newsub_SE,p_analysiss=p_analysiss,p_appeareds=p_appeareds,pass_percps=pass_percps, subwise_IA=subwise_IA,subwise_P=subwise_P,subwise_SE=subwise_SE,fc=fc,sclass=sclass,pclass=pclass,data = data)
